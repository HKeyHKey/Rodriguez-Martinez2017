...
ech2_Mixa <- read.table("Data/Mix_replicate1.txt", sep="\t", dec=".", header=TRUE)
ech2_Mixa <- calc.runmed2(ech2_Mixa, window = 5, data.M = "M.NORM", nom = "M.NORM.w")
ech2_Mixa <- calc.pvalue(ech2_Mixa, data.M = "M.NORM", window = 5, method = "mode.mad", plot = TRUE, adjust = FALSE)
safe.dir.create("Data_ce6_res")
write.table(ech2_Mixa, file = "Data_ce6_res/ech2.Mixa.55226301.control.neg.txt", quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
