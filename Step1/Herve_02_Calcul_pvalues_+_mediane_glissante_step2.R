...
ech2_40b <- read.table("Data/2-40_cells_replicate2.txt", sep="\t", dec=".", header=TRUE)
ech2_40b <- calc.runmed2(ech2_40b, window = 5, data.M = "M.NORM", nom = "M.NORM.w")
ech2_40b <- calc.pvalue(ech2_40b, data.M = "M.NORM", window = 5, method = "mode.mad", plot = TRUE, adjust = FALSE)
safe.dir.create("Data_ce6_res")
write.table(ech2_40b, file = "Data_ce6_res/ech2.40b.68238305.control.neg.txt", quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
